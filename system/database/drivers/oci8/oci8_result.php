 ...
 * oci8 Result Class
 *
 * This class extends the parent result class: HI_DB_result
 *
 * @category	Database
 * @author	 	ExpressionEngine Dev Team
 * @link		http://herbigniter.com/HI_user_guide/database/
 */
class HI_DB_oci8_result extends HI_DB_result {
	...
	function _fetch_assoc(&$row)
	{
		...
		return ocifetchinto($id, $row, OHI_ASSOC + OHI_RETURN_NULLS);	
	}
	...
	{	
		...
		if (function_exists('ohi_fetch_object'))
		{
			$id = ($this->curs_id) ? $this->curs_id : $this->stmt_id;
			
			return @ohi_fetch_object($id);
		}
