 ...
 * @category	Libraries
 * @author		ExpressionEngine Dev Team
 * @link		http://herbigniter.com/HI_user_guide/general/profiling.html
 */
class HI_Profiler {

	var $HI;
 	
 	function HI_Profiler()
 	{
	...
	function _compile_queries()
	{
		...
		foreach (get_object_vars($this->CI) as $HI_object)
		{
			if (is_object($HI_object) && is_subclass_of(get_class($HI_object), 'HI_DB') )
			{
				$dbs[] = $HI_object;
			}
		}
	...
	function run()
	{
		$output = "<div id='herbigniter_profiler' style='clear:both;background-color:#fff;padding:10px;'>";
